        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if do_Dr1:
                    ...
                    # Gradient penalty
                    # epsilon = torch.rand(real_img.shape[0], 1, 1, 1, device=real_img.device, requires_grad=True)
                    # interp_img = epsilon * real_img + (1 - epsilon) * gen_img.detach()
                    # interp_logits = self.run_D(interp_img, real_c, sync=False)
                    # grad_outputs = torch.ones_like(interp_logits)
                    # gradients = torch.autograd.grad(outputs=interp_logits, inputs=interp_img,
                    #                     grad_outputs=grad_outputs, create_graph=True,
                    #                     retain_graph=True, only_inputs=True)[0]
                    # gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean()
                    # training_stats.report('Loss/gradient_penalty', gradient_penalty)
                    # loss_gradient_penalty = self.lambda_gp * gradient_penalty
